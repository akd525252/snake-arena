...
import { api, Bep20Deposit, Bep20Wallet } from '../../../lib/api';
import { useI18n } from '../../../contexts/I18nContext';
import LanguageSwitcher from '../../../components/LanguageSwitcher';

export default function Bep20DepositPage() {
  const { t } = useI18n();
  const [wallet, setWallet] = useState<Bep20Wallet | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [copied, setCopied] = useState(false);
  const [deposits, setDeposits] = useState<Bep20Deposit[]>([]);
  const [pendingDeposits, setPendingDeposits] = useState<Bep20Deposit[]>([]);
  const [recentConfirmed, setRecentConfirmed] = useState<Bep20Deposit[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const w = await api.getBep20Wallet();
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-md w-full mx-auto px-6 py-10">
        ...
        <p className="rpg-text-muted mb-6 text-sm">
          Send USDT (BEP20) to your personal wallet address below. Your balance will be credited automatically.
        </p>
          ...
          <div className="space-y-6">
            ...
            <div className="rpg-panel p-6 text-center">
              <div className="text-xs rpg-text-muted mb-1 uppercase tracking-wider font-bold">
                Your USDT (BEP20) Deposit Address
              </div>
              ...
              <div className="mt-4 space-y-1">
                <div className="flex items-center justify-center gap-2 text-xs">
                  ...
                  <span className="rpg-text">Network: <span className="rpg-gold-bright font-bold">BEP20 (BSC)</span></span>
                </div>
                <div className="flex items-center justify-center gap-2 text-xs">
            ...
            <div className="rpg-panel p-4 border-[#962323]">
              ...
              <ul className="text-xs rpg-text-muted space-y-1 list-disc list-inside">
                <li>Send <strong>only USDT</strong> on the <strong>BEP20 (BSC)</strong> network</li>
                <li>Sending other tokens will result in <strong>permanent loss</strong></li>
                <li>Balance is credited after ~15 confirmations (~45 seconds)</li>
              ...
              <div className="rpg-panel p-4 border-[#d4a04a]">
                ...
                {pendingDeposits.map(dep => (
                  <div key={dep.id} className="flex justify-between items-center py-1">
                    ...
                    <span className="text-xs rpg-text-muted">
                      {dep.confirmations}/15 confirmations
                    </span>
                  </div>
                ))}
                <div className="h-2 rpg-parchment-inset rounded-full overflow-hidden mt-2">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, ((pendingDeposits[0]?.confirmations || 0) / 15) * 100)}%`,
                    }}
                  />
                </div>
              </div>

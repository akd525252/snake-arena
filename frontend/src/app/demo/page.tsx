...
function DemoPageInner() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Demo banner */}
      <div className="bg-[#ffb800]/10 border-b border-[#ffb800]/20 px-4 py-1.5 text-center text-xs">
        <span className="text-[#ffb800] font-bold">DEMO MODE</span>
        <span className="text-[#ffcc33]/60 ml-2">Playing with fake $50 demo balance</span>
      </div>
      ...
      <div className="absolute top-20 left-1/2 transform -translate-x-1/2 z-20 pointer-events-none">
        <div className={`px-6 py-2 rounded-full border-2 font-mono font-bold text-lg shadow-lg transition-colors duration-300 ${
          timeRemaining <= 10000 && timeRemaining > 0
            ? 'bg-[#ff2e63]/20 border-[#ff2e63] text-white animate-pulse'
            : 'bg-[#0a0a12]/95 border-[#ffb800]/50 text-white'
        }`}>
      ...
      <div className="px-6 py-3 bg-[#0a0a12] border-b border-[#1a1a2e] flex justify-between items-center text-sm">
        ...
        <div className="flex gap-6 text-[#8a8a9a] items-center">
          <span className="px-3 py-1 rounded-full bg-[#ffb800]/10 text-[#ffb800] text-xs font-bold">
            DEMO
          </span>
          <span className="hidden md:inline">Mouse = Steer · SPACE = Boost · SHIFT = Trap</span>
          <span className={`px-2 py-0.5 rounded-full text-xs ${
            status === 'connected' ? 'bg-[#ffb800]/20 text-[#ffb800]' :
            status === 'error' ? 'bg-[#ff2e63]/20 text-[#ff2e63]' :
            'bg-[#11111a] text-[#b0b0c0]'
          }`}>{status}</span>
        </div>
      </div>
      ...
      {showQuitConfirm && !results && !deathInfo && (
        <div className="absolute inset-0 bg-black/80 backdrop-blur flex items-center justify-center z-50">
          <div className="w-full max-w-md mx-6 p-8 rounded-2xl bg-[#0a0a12] border border-[#ff2e63]/30 text-center">
            <h2 className="text-2xl font-black mb-4 text-white">Leave Demo?</h2>
            ...
            <div className="flex justify-center gap-8 my-6">
              <div className="text-center">
                ...
                <p className="text-xl font-bold text-[#ffb800]">${betAmount.toFixed(2)}</p>
              </div>
              <div className="text-center">
        ...
        <div className="absolute inset-0 bg-black/90 backdrop-blur flex items-center justify-center z-50">
          <div className="w-full max-w-md mx-6 p-8 rounded-2xl bg-[#0a0a12] border border-[#ff2e63]/30 text-center">
            ...
            {deathInfo.killerName && (
              <p className="text-sm text-[#8a8a9a] mb-6">Killed by <span className="text-[#ffb800] font-bold">{deathInfo.killerName}</span></p>
            )}
            {!deathInfo.killerName && <p className="text-sm text-[#8a8a9a] mb-6">You hit the wall</p>}
            <div className="flex flex-col gap-3">
              <button
                onClick={() => window.location.reload()}
                className="w-full py-3 rounded-lg bg-[#ffb800] text-[#05050a] font-bold hover:bg-[#ffcc33] transition-colors"
              >
                Play Again
              </button>
              <Link
      ...
      {results && (
        <div className="absolute inset-0 bg-black/80 backdrop-blur flex items-center justify-center z-50">
          <div className="w-full max-w-md mx-6 p-8 rounded-2xl bg-[#0a0a12] border border-[#ffb800]/30">
            <h2 className="text-3xl font-black mb-2 text-center text-white">Demo Complete</h2>
            <p className="text-center text-xs text-[#6a6a7a] mb-6">These are demo earnings — not withdrawable</p>
            <div className="space-y-2 mb-6">
              {results.map((r, i) => (
                <div
                  key={i}
                  className={`flex justify-between items-center p-3 rounded-lg ${
                    r.placement === 1 ? 'bg-[#ffb800]/10 border border-[#ffb800]/30' : 'bg-[#11111a]/50'
                  }`}
                >
                  ...
                  <span className="text-[#ffb800] font-bold">${r.score.toFixed(2)}</span>
                </div>
              ))}
            </div>
            <div className="flex gap-3">
              <button
                onClick={() => window.location.reload()}
                className="flex-1 py-3 rounded-lg bg-[#ffb800] text-[#05050a] font-bold hover:bg-[#ffcc33] transition-colors"
              >
                Play Again
              </button>
              <Link
                href="/dashboard"
                className="flex-1 text-center py-3 rounded-lg border border-[#1a1a2e] hover:bg-[#11111a] text-white transition-colors"
              >
                Dashboard
              </Link>
            </div>
